...
def html_line_parser(line):
    return line.split('>', 1)[1][:-6]


def class_page_parser(data, crn):
    data = data.split('\n')
    class_data = {}
    ...
    for i in range(len(data)):
        if '<td align="center" valign="center">' in data[i]:
            for f in range(len(line_incr)):
                class_data[line_name[f]] = html_line_parser(data[i + line_incr[f]])
            break
    class_data['CRN'] = crn
    return class_data
...
class TMS:
    ...
    def download_class(self, crn, quarter):
        ...
        jsessionid = tms_homepage[0][1].split(';')[1][
                     tms_homepage[0][1].split(';')[1].index('=') + 1:tms_homepage[0][1].split(';')[1].index('?')]
        ...
        return class_page_parser(response.content.decode(), crn)
    ...
    # Takes in all the class info that has been parsed from the copied input
